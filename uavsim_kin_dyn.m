...
function out = uavsim_kin_dyn(uu,P)
    ...
    rates_mat = [1 sin(phi)*tan(theta) cos(phi)*tan(theta);0 cos(phi) -sin(phi); 0 sin(phi)*sec(theta) cos(phi)*sec(theta)];
    % Compute DCMs
    R_ned2b = eulerToRotationMatrix(phi,theta,psi);

    % NED Position EoMs
    R_b2ned = R_ned2b';
    Pdot_ned = R_b2ned*vg_b;

    % Body groundspeed vector EoMs
    vgdot_b = -cross(w_b,vg_b) + (1/mass)*f_b;

    % Euler angle EoMs
    euler_rates = rates_mat*w_b;

...
%   
    ...
    for i=1:length(x0)
        ...
        xdot_pert = eval_forces_moments_kin_dyn(x_perturbed,u0,P);
        A(:,i)= ( xdot_pert - xdot0 ) / eps_perturb;
    end
    ...
    for i=1:length(u0)
        ...
        udot_pert = eval_forces_moments_kin_dyn(x0,u_perturbed,P);
        B(:,i)= ( udot_pert - xdot0 ) / eps_perturb;
...
% non-linear equations of motion at the inputted condition (x & deltas).
    ...
    uu = [wind_ned(1:3); deltas(1:4); x(1:12); time(1)]; % Note: uu should be a column vector
    ...
    uu = [x(1:12); f_and_m(1:6); time(1)]; % Note: uu should be a column vector
